...
export function PlannerTimeline({ C, events, nowMin, isToday, onClickSlot, onClickEvent }: TimelineProps) {
 ...
 // ── 🛠️ HELPER MOUSE & TOUCH (Mendukung koordinat Jari HP dan Mouse PC) ──
 ...
 // ── DRAG START ──
 function startDragLogic(clientY: number, currentTargetTop: number, ev: PlannerEvent) {
   setDragging({ id: ev.id, offsetY: clientY - currentTargetTop })
   isDirty.current = false
 }

 // ── RESIZE START ──
 function startResizeLogic(evId: string) {
   setResizing(evId)
   isDirty.current = false
 }

 // ── DRAG & RESIZE MOVE (Gabungan Jari & Mouse) ──
 const handleMoveLogic = useCallback((clientY: number) => {
 ...
 // ── DRAG & RESIZE END (Simpan otomatis ke Supabase) ──
 const handleEndLogic = useCallback(async () => {
   if (isDirty.current) {
 ...
 return (
   <div
     ...
     style={{
       ...
       touchAction: dragging || resizing ? 'none' : 'auto', // 🛠️ Mencegah layar HP ikut tergulung saat sedang men-drag jadwal
     }}
     className="scrollbar-none"
     onClick={onTimelineClick}
     
     // Event Mouse Komputer
     onMouseMove={(e) => handleMoveLogic(e.clientY)}
     onMouseUp={handleEndLogic}
     onMouseLeave={handleEndLogic}
     
     // 🛠️ TAMBAHKAN: Event Sentuhan Jari HP Android/iOS
     onTouchMove={(e) => {
       if (dragging || resizing) {
         handleMoveLogic(e.touches[0].clientY)
       }
     }}
     onTouchEnd={handleEndLogic}
     onTouchCancel={handleEndLogic}
   >
     {/* Hour rows */}
     {hours.map(h => (
       ...
       {events.map(ev => {
         ...
         return (
           <div
             key={ev.id}
             // Pemicu Drag PC & HP
             onMouseDown={e => { e.stopPropagation(); const rect = (e.currentTarget as HTMLElement).getBoundingClientRect(); startDragLogic(e.clientY, rect.top, ev) }}
             onTouchStart={e => { e.stopPropagation(); const rect = (e.currentTarget as HTMLElement).getBoundingClientRect(); startDragLogic(e.touches[0].clientY, rect.top, ev) }}
           ...
           >
             ...
             {/* Resize handle (Grip Bawah) */}
             <div
               onMouseDown={e => { e.stopPropagation(); startResizeLogic(ev.id) }}
               onTouchStart={e => { e.stopPropagation(); startResizeLogic(ev.id) }}
               style={{
                 position: 'absolute', bottom: 0, left: 0, right: 0, height: 14, // Diperlebar jadi 14 agar lebih gampang disentuh jari HP
                 cursor: 'ns-resize',
                 display: 'flex', alignItems: 'center', justifyContent: 'center',
                 zIndex: 15,
               }}
             >
               <div style={{ width: 24, height: 4, borderRadius: 99, background: ev.color, opacity: .6 }} />
             </div>
           </div>
         )
       })}
