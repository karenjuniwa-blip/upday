import { useState } from 'react' // 🛠️ TAMBAHKAN useState
...
export function HabitDetailModal({ C, habit, onClose }: HabitDetailModalProps) {
 ...
 // 🛠️ TAMBAHKAN: State untuk mengontrol tanggal aktif kalender evaluasi
 const [currentCalendar, setCurrentCalendar] = useState(() => new Date())
 const year     = currentCalendar.getFullYear() // 🛠️ UBAH: Mengikuti state kalender
 const month    = currentCalendar.getMonth()    // 🛠️ UBAH: Mengikuti state kalender
 
 const days     = daysInMonth(year, month)
 const firstDay = firstDayOfMonth(year, month)

 // 🛠️ TAMBAHKAN: Fungsi handler geser bulan
 const changeMonth = (direction: number) => {
   setCurrentCalendar(prev => new Date(prev.getFullYear(), prev.getMonth() + direction, 1))
 }
 ...
 return (
   <Sheet C={C} onClose={onClose} maxH="92dvh">
     ...
     <div style={{
       ...
       {/* 🛠️ UBAH Bagian ini: Tambahkan tombol navigasi bulan di sebelah kiri/kanan nama bulan */}
       <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
         <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
           ...
           <div style={{ display: 'flex', gap: 6 }}>
             <button onClick={() => changeMonth(-1)} style={{ background: 'transparent', border: 'none', color: habit.color, cursor: 'pointer', fontSize: 12, padding: '0 4px', fontWeight: 'bold' }}>◀</button>
             <button onClick={() => changeMonth(1)} style={{ background: 'transparent', border: 'none', color: habit.color, cursor: 'pointer', fontSize: 12, padding: '0 4px', fontWeight: 'bold' }}>▶</button>
           </div>
         </div>
